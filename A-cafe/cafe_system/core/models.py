...
class Ingredient(BaseTimeModel):
    ...
    class Meta:
        ordering = ['name']
        constraints = [
            models.CheckConstraint(condition=models.Q(current_qty__gte=0), name='ingredient_current_qty_gte_zero'),
            models.CheckConstraint(condition=models.Q(min_qty_alert__gte=0), name='ingredient_min_qty_gte_zero'),
        ]
...
class Recipe(BaseTimeModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['product', 'ingredient'], name='unique_recipe_product_ingredient'),
            models.CheckConstraint(condition=models.Q(qty_per_item__gt=0), name='recipe_qty_per_item_gt_zero'),
        ]
...
class Order(BaseTimeModel):
    ...
    class Meta:
        ordering = ['-created_at']
        constraints = [
            models.CheckConstraint(condition=models.Q(subtotal__gte=0), name='order_subtotal_gte_zero'),
            models.CheckConstraint(condition=models.Q(tax_amount__gte=0), name='order_tax_amount_gte_zero'),
            models.CheckConstraint(condition=models.Q(discount_amount__gte=0), name='order_discount_amount_gte_zero'),
            models.CheckConstraint(condition=models.Q(grand_total__gte=0), name='order_grand_total_gte_zero'),
        ]
...
class StockReservation(BaseTimeModel):
    ...
    ingredient = models.ForeignKey(Ingredient, on_delete=models.PROTECT, related_name='stock_reservations')
    reserved_qty = models.DecimalField(max_digits=12, decimal_places=3)
    status = models.CharField(
        max_length=20, choices=ReservationStatusChoices.choices, default=ReservationStatusChoices.HELD
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(reserved_qty__gt=0), name='stock_reservation_reserved_qty_gt_zero'
            )
        ]

    def __str__(self):
        return f'{self.order} - {self.ingredient.name}: {self.reserved_qty}'
...
class Payment(BaseTimeModel):
    ...
    def __str__(self):
        return f'{self.order} - {self.method} NPR {self.amount}'
...
class Shift(BaseTimeModel):
    ...
    status = models.CharField(max_length=20, choices=ShiftStatusChoices.choices, default=ShiftStatusChoices.OPEN)
    notes = models.TextField(blank=True)

    class Meta:
        ordering = ['-opened_at']
        constraints = [
            models.CheckConstraint(condition=models.Q(opening_cash__gte=0), name='shift_opening_cash_gte_zero'),
            models.CheckConstraint(condition=models.Q(closing_cash_expected__gte=0), name='shift_closing_cash_expected_gte_zero'),
            models.CheckConstraint(condition=models.Q(closing_cash_actual__gte=0), name='shift_closing_cash_actual_gte_zero'),
        ]
